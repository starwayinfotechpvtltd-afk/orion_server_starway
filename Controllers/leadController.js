...
// Get leads generated by a specific manager's team members
export const getTeamLeads = async (req, res) => {
  try {
    const managerId = req.user.id;

    // 1. Find the team managed by the logged-in user
    const team = await TeamModel.findOne({ manager: managerId })
      .populate("members", "username email");

    if (!team) {
      // If the user doesn't manage a team, return null/empty
      return res.status(200).json({ team: null, leads: [] });
    }

    // 2. Extract the ObjectIds of the team members (callers)
    const memberIds = team.members.map((member) => member._id);
    
    // Optional: Include the manager's own ID if they also add leads
    memberIds.push(managerId);

    // 3. Fetch leads where the creator (userId) matches any of the member IDs
    const leads = await Lead.find({ userId: { $in: memberIds } })
      .populate("assignedTo", "username email")
      .populate("assignedBy", "username email")
      .sort({ createdAt: -1 });

    res.status(200).json({ team, leads });
  } catch (error) {
